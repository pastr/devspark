...
export function addCopyFileNameButtonToComments(): void {
  const comments = document.querySelectorAll<HTMLAnchorElement>("summary[role=button] a");

  comments.forEach((element) => {
    console.log("🚀 ~ file: index.ts:6 ~ comments.forEach ~ element:", element.text);
    const clipboardCopy = document.createElement("clipboard-copy");
    const valueToCopy = element.text?.split("/").at(-1);
    ...
    clipboardCopy.style.setProperty("color", "var(--color-btn-outline-hover-text)", "important");
    ...
    clipboardCopy.addEventListener("click", (e) => {
      console.log("🚀 ~ file: index.ts:31 ~ clipboardCopy.addEventListener ~ e:", e);
      e.preventDefault();
