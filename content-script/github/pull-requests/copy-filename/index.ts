export function addCopyFileNameButton(): void {
  const filesnames = document.querySelectorAll("a[title].Link--primary");

  filesnames.forEach((element) => {
    const clipboardCopy = document.createElement("clipboard-copy");
    const valueToCopy = element.getAttribute("title")?.split("/").at(-1);
